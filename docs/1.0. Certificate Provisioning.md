...
What about provisioning of the network's Root CA to NMOS Clients?
...
(Worth linking to RFC 7030 Section 4.2.3 to be clear this is an invention of that spec, not NMOS? I had to check.)

The EST Server MUST not support Basic HTTP Authentication.

The EST Server MUST be capable of issuing TLS Certificates signed with both (rephrase?) RSA and ECDSA Digital Signature algorithms.
