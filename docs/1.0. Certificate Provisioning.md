...
5. The EST Client should(?) send each CSR to the EST Server.
   - The EST Client must(?) provide the manufacturer-issued certificate during the TLS handshake, if this certificate is valid.
   - The EST Server will use the TLS Client Certificate to authenticate and then authorise whether the EST Client (what?). If the EST Client meets the local CA's security policy, it will be issued with a TLS Server Certificate. (How come RFC 7030 doesn't mention server certs??)
