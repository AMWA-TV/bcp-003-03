...
If an EST Client implements the NMOS IS-04 Node API, then the EST Client SHOULD advertise that it supports BCP-003-03 using the 'caps' attribute of the Node API. Control interfaces can identify all Nodes which support BCP-003-03 by a key 'cert' and Uniform Resource Name (URN) having the base 'urn:x-nmos:control:cert' followed by a '/' character and the supported version of this BCP.

**Example:** The 'caps' attribute of a NMOS Node
```
...
"caps": {
  "cert": "urn:x-nmos:control:cert/v1.0",
}
...
```
