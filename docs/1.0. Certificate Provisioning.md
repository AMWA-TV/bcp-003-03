...
If an EST Client implements the AMWA IS-04 Node API, then the EST Client SHOULD advertise that it supports BCP-003-03 using the 'tags' attribute of the Node API's `/self` endpoint. Control interfaces can identify all Nodes which support BCP-003-03 by a URN key 'urn:x-nmos:tag:certprov/', followed by the supported version of this BCP. When EST is enabled the value is set to `["enabled"]` and when disabled `["disabled"]`.

**Example:** The 'tags' attribute of a NMOS Node
```json
...
"tags": {
  "urn:x-nmos:tag:certprov/v1.0": ["enabled"]
}
...
```
